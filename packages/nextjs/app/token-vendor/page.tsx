...
const TokenVendor: NextPage = () => {
  ...
  console.log("Vendor Token Balance:", vendorTokenBalance);
      ...
      <div className="flex items-center flex-col flex-grow pt-10">
        ...
        {/* Buy Tokens */}
         <div className="flex flex-col items-center space-y-4 bg-base-100 shadow-lg shadow-secondary border-8 border-secondary rounded-xl p-6 mt-8 w-full max-w-lg">
          ...
          <div>{tokensPerEth ? tokensPerEth.toString() : "0"} tokens per ETH</div>
          ...
          <button
            ...
            onClick={async () => {
              try {
                // @ts-ignore
                await writeVendorAsync({ functionName: "approve", args: [vendorContractData?.address, getTokenPrice(tokensToBuy, tokensPerEth)] });
              } catch (err) {
              ...
              onClick={async () => {
                try {
                  await writeYourTokenAsync({
                    ...
                    args: [`0x${toAddress}`, multiplyTo1e18(tokensToSend)],
                  });
                } catch (err) {
